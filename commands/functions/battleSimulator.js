...
const det_expYield = require('./det_expYield.js');
...
module.exports = async function battleSimulator(Discord, bot, message, battle, profile, location, party, oppParty) {
    ...
    if (!winner) bot.commands.get('heal').execute(message);
    ...
    async function generateBattleImage() {
        ...
        if (active.shiny) {
            ...
            opponentImagePath = `./images/pokemon/pseudosprites/${opponent.spriteName}.png`;
        if (opponent.shiny) {
        ...
        try {
            ...
            for (let i = 0; i < moveEmojis.length; i++) {
                if (moveEmojis[i] === collected.first().emoji.name && moveEmojis[i] !== '❎') {
                    moveChoice = moveOptions[i].slice(4);
    ...
    async function battleInitiate(activeMove) {
        ...
        let foeType = 'The wild ';
        if (battle.opponent.type !== 'Wild') {
            foeType = "The opposing ";
        }
            ...
            switch (item[1]) {
                        ...
                        if (battle.active.boosts[item[3]] < 6) {
                            switch (item[4]) {
                                case '1':
                                    ...
                                    turnSummary.push(`${activeName}'s ${stat} rose! [${battle.active.boosts[item[3]]}]`);
                                    break;
                                case '2':
                                    battle.active.boosts[item[3]] += 2;
                                    if (battle.active.boosts[item[3]] > 6) battle.active.boosts[item[3]] = 6;
                                    turnSummary.push(`${activeName}'s ${stat} rose sharply! [${battle.active.boosts[item[3]]}]`);
                                    break;
                                case '3':
                                case '4':
                                case '5':
                                case '6':
                                    ...
                                    if (battle.active.boosts[item[3]] > 6) battle.active.boosts[item[3]] = 6;
                                    turnSummary.push(`${activeName}'s ${stat} rose drastically! [${battle.active.boosts[item[3]]}]`);
                                    break;
                            }
                        }
                        else {
                            turnSummary.push(`${activeName}'s ${stat} won't go any higher!`);
                        }
                    }
                    else {
                        if (battle.opponentActive.boosts[item[3]] < 6) {
                            switch (item[4]) {
                                case '1':
                                    ...
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} rose! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                                case '2':
                                    battle.opponentActive.boosts[item[3]] += 2;
                                    if (battle.opponentActive.boosts[item[3]] > 6) battle.opponentActive.boosts[item[3]] = 6;
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} rose sharply! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                                case '3':
                                case '4':
                                case '5':
                                case '6':
                                    ...
                                    if (battle.opponentActive.boosts[item[3]] > 6) battle.opponentActive.boosts[item[3]] = 6;
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} rose drastically! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                            }
                        }
                        else {
                            turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} won't go any higher!`);
                        }
                    }
                    break;
                ...
                case '-damage':
                    ...
                    if (item[2].split(':')[0] === 'p1a') {
                        ...
                        pokeName = `${foeType}${opponent.pokemon}`;
                    }

                    // If hurt by something other than a move
                    if (item[4]) {
                        switch (item[4].split('[from] ')[1]) {
                            case 'psn':
                                turnSummary.push(`${pokeName} was hurt by poison!`);
                                break;
                            default:
                                turnSummary.push(`${pokeName} was hurt by something undefined!`);
                                break;
                        }
                    }
                ...
                case 'faint':
                    if (item[2].split(':')[0] === 'p1a') {
                        ...
                        turnSummary.push(`${foeType}${opponent.pokemon} fainted!`);
                        turnSummary.push(`-------------------------`);
                ...
                case '-heal':
                    ...
                    if (item[2].split(':')[0] === 'p1a') {
                        ...
                        opponent.currentHP = tempHP;
                        turnSummary.push(`${foeType}${opponent.pokemon} recovered ${dmgHealed}HP!`);
                    }
                    break;
                ...
                case '-miss':
                    if (item[3].split(':')[0] === 'p1a') {
                        ...
                        turnSummary.push(`${foeType}${opponent.pokemon} avoided the attack!`);
                    }
                    break;
                ...
                case 'move':
                    if (item[2].split(':')[0] === 'p1a') {
                        ...
                        turnSummary.push(`${foeType}${opponent.pokemon} used ${opponent.moves[oppMove-1]}!`);
                    }
                    break;
                ...
                case '-status':
                    var pokeName = null;
                    if (item[2].split(':')[0] === 'p1a') pokeName = `${activeName}`;
                    else pokeName = `${foeType}${opponent.pokemon}`;
                    ...
                    switch (item[3]) {
                        ...
                        case 'par':
                            // X is paralyzed!
                            // It can't move!
                            turnSummary.push(`${pokeName} was paralyzed!`);
                        case 'psn':
                            turnSummary.push(`${pokeName} was poisoned!`);
                            break;
                        case 'tox':
                            turnSummary.push(`${pokeName} was badly poisoned!`);
                            break;
                        default:
                            turnSummary.push(`${pokeName} was afflicted with an undefined status!`);
                    }
                ...
                case 'tie':
                    ...
                    if (profile.lastHealLocation === 'leddintown') {
                        ...
                        const newLocation = await Location.findOne({_id: profile.lastHealLocation});
                    ...
                    if (item[2].split(':')[0] === 'p1a') {
                        if (battle.active.boosts[item[3]] > -6) {
                            switch (item[4]) {
                                case '1':
                                    battle.active.boosts[item[3]] -= 1;
                                    turnSummary.push(`${activeName}'s ${stat} fell! [${battle.active.boosts[item[3]]}]`);
                                    break;
                                case '2':
                                    battle.active.boosts[item[3]] -= 2;
                                    if (battle.active.boosts[item[3]] < -6) battle.active.boosts[item[3]] = -6;
                                    turnSummary.push(`${activeName}'s ${stat} harshly fell! [${battle.active.boosts[item[3]]}]`);
                                    break;
                                case '3':
                                case '4':
                                case '5':
                                case '6':
                                    battle.active.boosts[item[3]] -= parseInt(item[4], 10);
                                    if (battle.active.boosts[item[3]] < -6) battle.active.boosts[item[3]] = -6;
                                    turnSummary.push(`${activeName}'s ${stat} severely fell! [${battle.active.boosts[item[3]]}]`);
                                    break;
                            }
                        }
                        else {
                            turnSummary.push(`${activeName}'s ${stat} won't go any lower!`);
                        }
                    }
                    else {
                        if (battle.opponentActive.boosts[item[3]] > -6) {
                            switch (item[4]) {
                                case '1':
                                    battle.opponentActive.boosts[item[3]] -= 1;
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} fell! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                                case '2':
                                    battle.oppopnentActive.boosts[item[3]] -= 2;
                                    if (battle.opponentActive.boosts[item[3]] < -6) battle.opponentActive.boosts[item[3]] = -6;
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} harshly fell! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                                case '3':
                                case '4':
                                case '5':
                                case '6':
                                    battle.opponentActive.boosts[item[3]] -= parseInt(item[4], 10);
                                    if (battle.opponentActive.boosts[item[3]] < -6) battle.opponentActive.boosts[item[3]] = -6;
                                    turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} severely fell! [${battle.opponentActive.boosts[item[3]]}]`);
                                    break;
                            }
                        }
                        else {
                            turnSummary.push(`${foeType}${opponent.pokemon}'s ${stat} won't go any lower!`);
                        }
                    }
                    break;
    ...
    function awardExp(pokemon, position) {
        ...
        const det_stats = require('../pokeinfo/det_stats');
        const expLookup = require('../pokeinfo/experienceLookup');

        let expGained = null,
            a = 1,
            b = det_expYield(opponent.pokemon.toLowerCase()),
            e = 1,
            L = opponent.level,
            Lp = pokemon.level,
            p = 1,
        ...
        //console.log(`exp yield: ${b}`);
        //console.log(`${pokemon.pokemon} level: ${pokemon.level}`);
        //console.log(`Opposing ${opponent.pokemon} level: ${opponent.level}`);
        ...
        while (pokemon.exp.current > pokemon.exp.nextLevel) {
            ...
            pokemon.level += 1;
            leveled = true;
            let newStats = det_stats(pokemon.pokemon.toLowerCase(), pokemon.level, pokemon.natureMultipliers);
            deltaHP = newStats[0] - pokemon.maxHP;
            pokemon.maxHP = newStats[0];
            pokemon.currentHP += deltaHP;

            // update next level exp goal and percentage attained
            let expInfo = expLookup(pokemon.pokemon.toLowerCase(), pokemon.level, pokemon.exp.current);
            ...
            newMoveset = movesets(pokemon.pokemon.toLowerCase(), pokemon.level, newMoveset);
        } 
        if (leveled) {
            turnSummary.push(`${pokeName} grew to level ${pokemon.level}!`)
            if (pokemon.moves !== newMoveset) {
                for (const i = pokemon.moves.length; i < newMoveset.length; i++) {
